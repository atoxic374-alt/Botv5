...
// captchaRqtoken: the rqtoken Discord sent with the captcha challenge — MUST be echoed back.
// Returns { token, userId, needsPhone } on success, throws on failure.
async function registerAccount({ http, email, username, password, captchaKey, captchaRqtoken, fingerprint, buildNumber }) {
  ...
  // X-Captcha-Key header required by modern Discord in addition to body field.
  // X-Captcha-Rqtoken must echo the rqtoken Discord sent with the captcha challenge.
  const headers = {
  ...
  // Discord returned a new captcha challenge — pass all challenge fields back to caller
...
async function createOneAccountAsync({
  ...
  try {
    // Probe request: triggers the captcha challenge so we get sitekey + rqdata + rqtoken
    ...
    captchaRqtoken     = probe.data?.captcha_rqtoken || null;

    log('info', `نوع الكابتشا: ${rqdata ? 'hCaptcha Enterprise (rqdata)' : 'hCaptcha Standard'}`);
  ...
  // ── 7. Register — retry on username collision OR captcha re-challenge ─────
  ...
  while (true) {
    try {
      ({ token, userId, needsPhone } = await registerAccount({
        http, email, username, password, captchaKey, captchaRqtoken, fingerprint, buildNumber,
      }));
      break; // success
    } catch (e) {
      // ── Discord returned a new captcha challenge ─────────────────────────
      if (e.code === 'CAPTCHA' && captchaAttempt < MAX_CAPTCHA_RETRIES) {
        captchaAttempt++;
        captchaRqtoken = e.rqtoken || null;
        log('warn', `⚠ Discord طلب كابتشا جديدة (محاولة ${captchaAttempt}/${MAX_CAPTCHA_RETRIES})...`);
        try {
          ...
          log('info', `✅ الكابتشا محلول (محاولة ${captchaAttempt})`);
        } catch (captchaErr) {
          if (phoneId && smsProvider) await smsProvider.cancelNumber(phoneId, 'discord').catch(() => {});
          await emailProvider.deleteInbox(emailCtx).catch(() => {});
