...
// ── Usernames pool ───────────────────────────────────────────────────────────
const ADJECTIVES = ['Dark','Swift','Neon','Cool','Bright','Alpha','Silent','Iron','Storm','Ghost','Cyber','Ultra','Prime','Royal','Sharp'];
const NOUNS      = ['Wolf','Fox','Hawk','Bear','Lion','Eagle','Tiger','Panda','Lynx','Raven','Drake','Viper','Nova','Byte','Core'];
...
function generateUsername(prefix) {
  if (prefix && prefix.trim()) {
    return `${prefix.trim()}_${randStr(4)}`;
  }
  return `${randEl(ADJECTIVES)}${randEl(NOUNS)}${randBetween(10, 9999)}`;
}
